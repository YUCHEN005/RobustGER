...
class CausalSelfAttention(BaseCausalSelfAttention):
    ...
    def __init__(self, config: Config, block_idx: int) -> None:
        ...
        if block_idx >= config.adapter_start_layer:
            ...
            self.key_gating_factor = torch.nn.Parameter(torch.zeros(1, 32, 20, 128))
            self.value_gating_factor = torch.nn.Parameter(torch.zeros(1, 32, 20, 128))
    ...
    def forward(
        ...
        if self.block_idx >= self.config.adapter_start_layer:
            ...
            if adapter_kv_cache is not None:
                ...
                ak = ak.view(1, -1, aT, self.config.head_size).repeat(B, 1, 1, 1)  # (B, nh_ak, aT, hs) = (B, 32, 20, 128)
                av = av.view(1, -1, aT, self.config.head_size).repeat(B, 1, 1, 1)  # (B, nh_av, aT, hs)
...
class Block(nn.Module):
    ...
    def forward(
        ...
        h, emb_diff_mid, new_kv_cache, new_adapter_kv_cache = self.attn(
            n_1, emb_diff, rope, max_seq_length, mask, input_pos, kv_cache, adapter_kv_cache
        )
    ...
    def forward(
        ...
        if not use_kv_cache:
            for block in self.transformer.h:
                x, emb_diff_mid, *_ = block(x, emb_diff, (cos, sin), max_seq_length)
            ...
            for i, block in enumerate(self.transformer.h):
                x, emb_diff_mid, self.kv_caches[i], self.adapter_kv_caches[i] = block(
                    x, emb_diff, (cos, sin), max_seq_length, mask, input_pos, self.kv_caches[i], self.adapter_kv_caches[i]
                )
                emb_diff_mids.extend(emb_diff_mid)
...
def adapter_filter(key: str, value: Any) -> bool:
    return "adapter_wte" in key or "gating_factor" in key or 'projection' in key or 'ef_' in key
